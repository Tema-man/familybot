...
abstract class CommandExecutorTest : ExecutorTest() {
    ...
    override fun canExecuteTest() {
        ...
        val messageWithCommandInMiddle =
            createSimpleCommandContext(
                prefix = randomString(),
                postfix = randomString(),
                command = command
            )
        ...
        val messageForOtherBot =
            createSimpleCommandContext(command = command, postfix = "@${randomString()}")
        Assertions.assertFalse(
            commandExecutor.canExecute(messageForOtherBot),
            "Should not react for command which addressed to another bot"
        )
        val messageForSuchara =
            createSimpleCommandContext(command = command, postfix = "@IntegrationTests")
        Assertions.assertTrue(
